...
- Q: What network egress is the CLI permitted to make at runtime, beyond fetching the target URL? → A: Local-only — the CLI may fetch only the target URL and its transitively-loaded resources; all deterministic tools MUST run locally against the captured artifacts; no third-party scoring APIs and no package-registry calls in the runtime scoring path.
- Q: Which sub-criteria are "visual-parity" and become `not_applicable` when no reference image is supplied? → A: Each sub-criterion carries a `visual_parity: true|false` flag in the rubric definition. The rubric ships with this flag defaulting to `true` on every sub-criterion bound to a `pixelmatch.*` check; project config may override the flag per sub-criterion. Only flagged sub-criteria become `status = "not_applicable"` when no reference image is supplied.
- Q: How does the CLI ensure a stable, deterministic screenshot frame from a dynamic live page for pixel comparison? → A: CSS injection freeze — before every screenshot capture, the CLI injects a stylesheet that sets `animation-duration: 0s`, `animation-delay: 0s`, `transition-duration: 0s`, `transition-delay: 0s`, `caret-color: transparent`, `scroll-behavior: auto` on `*, *::before, *::after` (all `!important`), hides scrollbars and selection highlights, and forces scroll to top. This is applied automatically on every capture and is not configurable (always on for determinism).
- Q: How does the CLI handle inherently dynamic page content (timestamps, ads, carousels, avatars) that will always differ from the static reference and produce false-positive pixel diffs? → A: Config-based mask selectors — the project config accepts a `pixel_comparison.mask_selectors` list of CSS selectors. Before pixelmatch, the CLI locates those elements in the live page, paints them with a solid placeholder color, and applies identical masks at the same coordinates on the reference image. Masked pixels are excluded from diff calculation.
- Q: What provenance is expected for the reference image, and what contract governs its characteristics for a fair pixel comparison? → A: Accept any PNG (no provenance enforcement, no `capture-reference` command). Document a reference-image contract specifying required characteristics: pixel dimensions must equal the CLI's capture at the effective DPR (default 1); color space is sRGB; format is 8-bit PNG; transparent pixels are composited onto white (#FFFFFF) before comparison; RGB references have alpha filled to opaque. The CLI reports capture characteristics (DPR, viewport, color space) in the pixel_comparison output so the operator can verify alignment.
...
- **FR-005a**: The CLI MUST document and enforce a **reference-image contract** governing the characteristics required for a meaningful pixel comparison. The contract is: (1) **Pixel dimensions** — the reference image's width × height in pixels MUST equal the CLI's screenshot dimensions at the effective device pixel ratio (DPR); for a 1280×800 viewport at DPR 1 the reference must be 1280×800 px, at DPR 2 it must be 2560×1600 px. (2) **Device pixel ratio** — the CLI captures at a configurable DPR (default: `1`, configurable via `pixel_comparison.device_pixel_ratio` in the project config); the reference MUST be authored at the same DPR. (3) **Color space** — the CLI captures in sRGB; references in other color profiles (Display P3, Adobe RGB) are accepted but may inflate `diff_ratio` due to profile-conversion rounding. (4) **Format and depth** — 8-bit PNG; RGBA preferred. RGB references (no alpha channel) will have alpha filled to opaque (255) before comparison. Transparent pixels in the reference are composited onto a white (`#FFFFFF`) background before comparison, matching the browser's opaque-background rendering. (5) **Content state** — the reference represents the desired visual output; the operator is responsible for ensuring it reflects the intended settled render, consistent with the configured `mask_selectors` (masked regions need not match). The CLI MUST report the capture characteristics used (`device_pixel_ratio`, `viewport`, `color_space: "srgb"`) in the `pixel_comparison` output block so the operator can verify alignment with their reference. Any PNG is accepted regardless of provenance (design-tool export, prior screenshot, hand-crafted mockup); no metadata marker or CLI-generated provenance is required.
...
- **FR-007**: The CLI MUST render the live URL through a headless browser with a configurable settle policy (default: wait for network idle plus a small additional delay) before capturing artifacts. Immediately before every screenshot capture the CLI MUST inject a stabilization stylesheet that forces `animation-duration: 0s !important; animation-delay: 0s !important; transition-duration: 0s !important; transition-delay: 0s !important; caret-color: transparent !important; scroll-behavior: auto !important` on `*, *::before, *::after`, hides native scrollbars and selection highlights via pseudo-element overrides, and scrolls the page to the top. This stabilization is always-on and not operator-configurable — it is a prerequisite for the FR-003 determinism guarantee and for meaningful pixel comparison against a static reference image.
...
- **FR-017**: When a reference design image is supplied, the CLI MUST perform pixel-level comparison between the live UI's captured screenshot and the reference, using the `pixelmatch` library (https://github.com/mapbox/pixelmatch). Before comparison, the CLI MUST apply any mask selectors declared in the project configuration (see FR-017a) to exclude dynamic content regions from the diff calculation.
- **FR-017a**: The project configuration MAY declare a `pixel_comparison.mask_selectors` list of CSS selectors identifying page regions that contain inherently dynamic content (e.g., `[".ad-slot", ".timestamp", ".chat-widget", "[data-testid='avatar']"]`). When mask selectors are configured, the CLI MUST: (a) resolve each selector against the live page's DOM to obtain bounding boxes, (b) paint those bounding boxes with a solid placeholder color (default `#FF00FF`, configurable via `pixel_comparison.mask_color`) on the live screenshot before comparison, (c) apply identical masks at the same coordinates on the reference image so that both images are masked symmetrically, (d) report the number of masked pixels and the effective compared-pixel count in the `pixel_comparison` output. Masked pixels MUST be excluded from `diff_pixel_count` and `diff_ratio` calculations — i.e., `total_pixel_count` in the output represents only unmasked pixels. When no mask selectors are configured, the comparison proceeds against the full screenshot.
...
- **FR-019**: The CLI MUST map `diff_ratio` to a 0–4 score per the documented pixel-parity anchors and bind that score to every sub-criterion flagged `visual_parity: true` in the rubric definition. By default the rubric MUST ship with `visual_parity: true` set on every sub-criterion whose bound deterministic check belongs to the `pixelmatch.*` family; project configuration MAY override the flag per sub-criterion.
...
- **FR-022**: When no reference image is supplied, every sub-criterion with `visual_parity: true` (per FR-019) MUST be emitted with `score = null` and `status = "not_applicable"`; the composite MUST re-weight their parent dimensions per FR-012/FR-013; the run MUST still complete and produce the rest of the artifact.
...
- **FR-026b**: Runtime network egress is restricted to the target URL and its transitively-loaded resources (resources the headless browser fetches while rendering the target). Every bound deterministic check MUST execute locally against the captured artifacts; the CLI MUST NOT call a third-party scoring or analysis API (e.g., PageSpeed Insights, WebPageTest, hosted accessibility services), and MUST NOT reach a package registry, update server, or telemetry endpoint as part of the scoring path. Tool installation and rubric updates are out of scope for the runtime path and MUST be performed by a separate provisioning step before invocation.

#### Configuration

- **FR-027**: The CLI MUST accept a declarative project configuration file that can override default dimension weights, toggle `blocking_if_zero` on individual sub-criteria, add custom sub-criteria (each bound to a named deterministic check) under existing dimensions, select / configure the bound check for any sub-criterion, set viewport(s) for capture and pixel comparison, declare mask selectors for pixel comparison (per FR-017a), and set tool fallback policy.
...
- The CLI runs in an environment where a headless browser can be launched and where one or more accessibility / performance tools have been installed alongside the CLI by a separate provisioning step. Per FR-026b the only runtime network egress the CLI itself performs is to the target URL and its transitively-loaded resources; the environment can therefore restrict outbound traffic to just the target host without breaking the scoring path.
