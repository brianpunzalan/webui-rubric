...
export function checkHeadingOrder(html: string): StructuralCheckResult {
  ...
  return {
    score,
    evidence: `Heading hierarchy: ${skips} level skips detected in ${levels.length} headings`.slice(
      0,
      300,
    ),
...
export function checkLinkDescriptiveness(html: string): StructuralCheckResult {
  ...
  return {
    score,
    evidence: `${genericLinks}/${totalLinks} links (${pct.toFixed(1)}%) use generic text`.slice(
      0,
      300,
    ),
    evidence_source: 'dom.link-descriptiveness',
...
export function checkImageAlt(html: string): StructuralCheckResult {
  ...
  return {
    score,
    evidence:
      `${missingAlt}/${images.length} images (${pct.toFixed(1)}%) missing alt attribute`.slice(
        0,
        300,
      ),
    evidence_source: 'dom.image-alt',
...
export function checkFormLabels(html: string): StructuralCheckResult {
  ...
  return {
    score,
    evidence:
      `${unlabeled}/${inputs.length} inputs (${pct.toFixed(1)}%) without associated labels`.slice(
        0,
        300,
      ),
    evidence_source: 'dom.form-labels',
