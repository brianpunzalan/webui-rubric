...
export function checkConsoleErrors(consoleErrors: ConsoleEntry[]): RuntimeCheckResult {
  ...
  const score = errorCount === 0 ? 4 : errorCount <= 2 ? 3 : errorCount <= 5 ? 2 : errorCount <= 10 ? 1 : 0;
  return {
    score,
    evidence: `${errorCount} console errors detected during page load`,
    evidence_source: 'console.error-count',
    severity: 4 - score,
    suggested_fix: score < 4 ? `Fix console errors; ${errorCount} errors detected during page load` : '',
    location: null,
  };
}
...
export function checkResourceCount(har: unknown): RuntimeCheckResult {
  ...
  const score = requestCount <= 25 ? 4 : requestCount <= 40 ? 3 : requestCount <= 60 ? 2 : requestCount <= 100 ? 1 : 0;
  return {
    score,
    evidence: `${requestCount} network requests during page load`,
    evidence_source: 'har.resource-count',
    severity: 4 - score,
    suggested_fix: score < 4 ? `Reduce number of network requests; current count is ${requestCount} (target: ≤40)` : '',
    location: null,
  };
}
