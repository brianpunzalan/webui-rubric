...
- `--artifact-dir` now defaults to `.webui-rubric/artifacts`, so every `evaluate` run emits a bundle (data-only unless `--reference` is supplied). The directory is gitignored.
